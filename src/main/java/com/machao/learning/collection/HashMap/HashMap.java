...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    final Node<K,V>[] resize() {
        Node<K,V>[] oldTab = table;// 将当前数组赋值给oldTab
        int oldCap = (oldTab == null) ? 0 : oldTab.length;// 将当前数组容量赋值给oldCap
        int oldThr = threshold;// 将当前扩容阀值赋值给oldThr
        int newCap, newThr = 0;
        if (oldCap > 0) {// 当前Map不为空时
        	// 如果数组容量已达到最大容量，则不在扩容
            if (oldCap >= MAXIMUM_CAPACITY) {
                threshold = Integer.MAX_VALUE;
                return oldTab;
            }
            // 如果满足扩容条件，则将容量和扩容阀值都增加一倍
            ...
            // 使用默认容量16和默认加载因子0.75f计算阀值
            newThr = (int)(DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY);
        }
        // 新构造的Map指定了初始容量时，计算阀值
        if (newThr == 0) {
            float ft = (float)newCap * loadFactor;
            newThr = (newCap < MAXIMUM_CAPACITY && ft < (float)MAXIMUM_CAPACITY ?
                      (int)ft : Integer.MAX_VALUE);
        }
        threshold = newThr;// 将新的扩容阀值赋值给threshold
        // 初始化一个新容量的数组取代原来的table
        @SuppressWarnings({"rawtypes","unchecked"})
            Node<K,V>[] newTab = (Node<K,V>[])new Node[newCap];
        table = newTab;
        if (oldTab != null) {
        	// 从oldTab[1]开始遍历，因为数组中[0]位置存的是null值
            for (int j = 0; j < oldCap; ++j) {
                Node<K,V> e;
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null; // 帮助 GC
                    if (e.next == null) // 如果是桶中只有一个元素，重新计算桶的位置
