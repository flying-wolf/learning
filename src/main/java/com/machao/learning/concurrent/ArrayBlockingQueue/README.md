...
## 总结
> ArrayBlockingQueue是一个由数组结构支持的有界阻塞队列；
> 属性items是存放元素的数组，takeIndex为出队指针记录下一次出队操作在数组中的索引，putIndex为入队指针记录下一次入队操作在数组中的索引；     
> 此队列线程安全，内部使用一把互斥锁来保护竞争资源。   
> 出队操作以notEmpty作为空队列的阻塞条件，入队操作以notFull作为满队列的阻塞条件。
> add(e)方法：向队列尾部插入元素，成功返回true，队列已满抛出IllegalStateException异常。   
> put(e)方法：向队列尾部插入元素，如果队列满则阻塞，直到队列不满或当前线程被中断。
> offer(e)方法：向队列尾部插入元素，如果成功返回true，如果队列满返回false。   
> offer(e,timeout,unit)方法：向队列尾部插入元素，成功返回true，如果队列满则阻塞，直到队列不满或阻塞超时或当前线程被中断，阻塞超时返回false。    
> take()方法：检索并删除队列头元素，如果队列为空则阻塞，直到队列非空或当前线程被中断。   
> poll()方法：检索并删除队列头元素，如果队列为空则立即返回null。
> poll(timeout,unit)方法：检索并删除队列头元素，如果队列为空则阻塞，直到队列非空或阻塞超时或当前线程被中断，如果阻塞超时返回null。   
> peek()方法：检索队列头元素，如果队列为空返回null。   
> removeAt(i)方法：删除队列中指定索引位置的元素。   
> remove(o)方法：删除队列中指定元素的节点，成功返回true，如果队列为空或没有此元素的节点返回false。  
